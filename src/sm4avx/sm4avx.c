...
//SBOX_T0[] = L(sbox[])
const uint32_t _SM4_SBOX_T0_[256] = {
...
//SBOX_T1[] = rol(L(sbox[]),8) 
const uint32_t _SM4_SBOX_T1_[256] = {
...
//SBOX_T1[] = rol(L(sbox[]),2*8) 
const uint32_t _SM4_SBOX_T2_[256] = {
...
//SBOX_T1[] = rol(L(sbox[]),3*8)
const uint32_t _SM4_SBOX_T3_[256] = {
...
//对x86汇编ror循环左移指令的封装
static inline uint32_t _sm4_asm_rol_(uint32_t data, uint32_t bits)
{
    asm ("rol %1, %0;\n\t"
    : "+r" (data)
    : "cI" (bits));
...
//sm4 8道sbox函数
static inline __m256i sm4_t_8(__m256i data)
{
    __m256i t[4];
    __m256i mask;
    ...
    t[1] = _mm256_srai_epi32(data, 8);
    t[2] = _mm256_srai_epi32(data, 16);
    t[3] = _mm256_srai_epi32(data, 24);

    t[0] = _mm256_and_si256(data, mask);
    t[1] = _mm256_and_si256(t[1], mask);
    t[2] = _mm256_and_si256(t[2], mask);
    t[3] = _mm256_and_si256(t[3], mask);

    t[0] = _mm256_i32gather_epi32(_SM4_SBOX_T0_, t[0], 4);
    t[1] = _mm256_i32gather_epi32(_SM4_SBOX_T1_, t[1], 4);
    t[2] = _mm256_i32gather_epi32(_SM4_SBOX_T2_, t[2], 4);
    t[3] = _mm256_i32gather_epi32(_SM4_SBOX_T3_, t[3], 4);

    return _mm256_xor_si256(t[0], _mm256_xor_si256(t[1], _mm256_xor_si256(t[2], t[3])));
}
